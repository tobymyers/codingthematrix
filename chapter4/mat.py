# Copyright 2012 Philip N. Klein
from vec import Vec, getitem as vecget
from GF2 import *
...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    A.f = {(i,j):getitem(A,(i,j)) for i in A.D[0] for j in A.D[1]}
    B.f = {(i,j):getitem(B,(i,j)) for i in B.D[0] for j in B.D[1]}
    return A.f == B.f
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat((M.D[0], M.D[1]), {(i,j):M[i,j] * x for i in M.D[0] for j in M.D[1]})
def transpose(M):
    """
    ...
    True
    """
    return Mat((M.D[1], M.D[0]), {(i,j):getitem(M,(j,i)) for j in M.D[0] for i in M.D[1]})
...
def vector_matrix_mul(v, M):
    ...
    pre =({(i,j):getitem(M,(i,j))*vecget(v,V) for (i,V) in zip(M.D[0], v.D) for j in M.D[1]})
