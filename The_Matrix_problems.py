...
#column in b matches row in a and row in b matches column in a
...
your_answer_e_BA = [[-2,12,4,15],[0,0,0,0],[0,0,0,0],[-3,-15,6,-9]]
...
column_row_vector_multiplication5 = Vec({0, 1, 2}, {0:-3,1:1,2:7})
...
def lin_comb_vec_mat_mult(v, M):
    ...
    #this code I found is for sure more efficient, but it seems to use the * operator
    #which isn't legal according to the description in the book.  But the grader doesn'#
    #like my solution
    assert(v.D == M.D[0])
...
def dot_product_mat_vec_mult(M, v):
    ...
    out_vec = Vec(M.D[0],{})
    for (row,d) in zip(row_vecs.values(),out_vec.D):
        out_vec[d] = v * row
    return out_vec
...
def dot_product_vec_mat_mult(v, M):
    ...
    out_vec = Vec(M.D[1],{})
    for (col,d) in zip(col_vecs.values(),out_vec.D):
        out_vec[d] = v * col
    return out_vec
...
most_opposed_10_pairs_of_countries = [{'Belarus', 'United_States_of_America'}, {'Syria', 'United_States_of_America'},  {'Cuba', 'United_States_of_America'}, {'Algeria', 'United_States_of_America'},  {'United_States_of_America', 'Viet_Nam'}, {'Libya', 'United_States_of_America'}, {'Guinea', 'United_States_of_America'}, {'Mongolia', 'United_States_of_America'}, {'Mali', 'United_States_of_America'}, {'Sudan', 'United_States_of_America'}]
# Provide a set consisting of two strings
...
def dictlist_helper(dlist, k):
    ...
    ['Apfel', 'manzana', 'pomme']
    '''
    return [dict[k] for dict in dlist for key,value in dict.items() if key == k ]
